package fr.nextdigital.lab.account.worker.config;
...
import fr.nextdigital.lab.account.worker.function.*;
 ...
 * resource given a sequence of ordered {@link fr.nextdigital.lab.account.worker.event.AccountEvents}. When an event is received
 * in {@link fr.nextdigital.lab.account.worker.event.AccountEventStream}, an in-memory state machine is fully replicated given the
 * {@link fr.nextdigital.lab.account.worker.event.AccountEvents} attached to an {@link Account} resource.
 *
 * @author kbastani
 */
@Configuration
...
public class StateMachineConfig extends EnumStateMachineConfigurerAdapter<AccountStatus, AccountEventType> {
    ...
    final private Logger log = org.slf4j.LoggerFactory.getLogger(StateMachineConfig.class);
    ...
    public Action<AccountStatus, AccountEventType> confirmAccount() {
        return context -> {
            ...
            accountFunction = new ConfirmAccount(context, event -> {
                ...
                log.info(event.getType() + ": " + newLinkToPod);
