package fr.nextdigital.lab.payment.worker.config;
...
import fr.nextdigital.lab.payment.worker.function.*;
 ...
 * resource given a sequence of ordered {@link fr.nextdigital.lab.payment.worker.event.PaymentEvents}. When an event is received
 * in {@link fr.nextdigital.lab.payment.worker.event.PaymentEventStream}, an in-memory state machine is fully replicated given the
 * {@link fr.nextdigital.lab.payment.worker.event.PaymentEvents} attached to an {@link Payment} resource.
 *
 * @author kbastani
 */
@Configuration
